...
traits:
  ...
  - name: Hold
    desc: "Once a giant weasel bites a living creature, it hangs on, rending with its teeth each round until the victim or the weasel is dead, or until the weasel fails a DC 12 CHA check (rolled normally) in which case it will let go of its victim and flee."
actions:
  - name: bite + hold
    desc: "D20 to hit, 2d4 + 2d4 per round"
source: BFRPG Core
```
