...
traits:
  ...
  - name: Shell Retreat
    desc: "Each snail has but a single tooth to launch, and if this attack is not successful, the creature will retreat inside its shell. This gives it a bonus of +6 to its AC. The snail will then simply wait for any enemy combatants to give up and leave."
  - name: Tactical Hunter
    desc: "A giant cone snail generally will not attack groups of creatures which outnumber it, but may lay in wait for a single opponent to become isolated and thus vulnerable."
actions:
  - name: dart (1d6 + paralysis)
    desc: "D20 to hit, 1d6 (`dice:1d6`) + paralysis"
source: BFRPG Field Guide
```
