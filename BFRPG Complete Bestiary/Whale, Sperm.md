---
statblock: inline
name: Whale, Sperm
obsidianUIMode: preview
tags:
  - monster
aliases: []
source: BFRPG Core
---

```statblock
name: Whale, Sperm
layout: Shadowdark
ac: "21"
level: "36"
hp: 178
atk_bonus: "+16"
modifier: 9
stats: ["36", "178", "21", "near"]
attack: "+16 (`dice: 1d20+16`) 1 bite or special"
damage: "3d20 (`dice:3d20`)"
attributes: ["+11", "+9", "+10", "+9", "+9", "+9"]
traits:
  - name: Sonic Blast
    desc: "Can emit an invisible focused beam of sound 5' wide up to a 50' range underwater. This blast of sound disorients target creatures, leaving them effectively stunned for 1d4 rounds. A stunned character can neither move nor take action for the indicated duration. No attack roll is required, but a DC 19 CON check is allowed to resist. A sperm whale can emit as many such blasts of sound as it desires, once per round, instead of biting."
actions:
  - name: bite or special
    desc: "D20 to hit, 3d20 (`dice:3d20`)"
source: BFRPG Core
```

These creatures can be up to 60 feet long. They prey on giant squid. Sperm whales can emit an invisible focused beam of sound 5' wide up to a 50' range underwater. This blast of sound disorients target creatures, leaving them effectively stunned for 1d4 rounds. A stunned character can neither move nor take action for the indicated duration. No attack roll is required, but a DC 19 CON check is allowed to resist. A sperm whale can emit as many such blasts of sound as it desires, once per round, instead of biting.

