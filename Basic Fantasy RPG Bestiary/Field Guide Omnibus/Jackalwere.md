---
statblock: inline
name: Jackalwere
obsidianUIMode: preview
tags:
  - monster
aliases: []
source: Field Guide Omnibus
---

```statblock
name: Jackalwere
layout: BFRPG
subtype: Field Guide Omnibus
ac: 3 [16]
hit_dice: 4*
hp: 18
thaco: 16 [+4]
modifier: 0
stats: ["4*", "18", "3 [16]", "16 [+4]"]
attack: "1 bite or 1 weapon"
damage: "2d4 bite or by weapon"
speed: "60'"
jds: [13, 14, 13, 15, 14]
moral: "7"
xp: 280
nbr: "1d4"
loot: "C"
roll_jds: 1d20
roll-moral: 2d6
traits:
  - name: Shapeshifting
    desc: "Can assume the form of a somewhat larger-than-normal jackal or that of a man."
  - name: Sleep Gaze
    desc: "The ability to cause a living creature who meets its gaze to fall asleep. This effect is otherwise as the sleep spell, and can be used any number of times per day. Note that any character facing a jackalwere who is aware of the creature's identity and knows of this power can easily avoid its gaze; only a truly unsuspecting opponent will be affected."
  - name: Pack Leadership
    desc: "Rarely (1 on 1d6) found leading a pack of ordinary jackals."
actions:
  - name: bite or 1 weapon
    desc: "D20 to hit, 2d4 bite or by weapon"
source: Field Guide Omnibus
```

Despite the name, Jackalwere are not ordinary lycanthropes; their condition is not contagious, for example. These strange creatures can assume the form of a somewhat larger-than-normal jackal or that of a man. They are rarely (1 on 1d6) found leading a pack of ordinary jackals, but otherwise will be hunting by themselves.

